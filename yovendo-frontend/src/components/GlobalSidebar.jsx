...
export default function GlobalSidebar() {
  ...
  const navItems = [
    ...
    user.role === 'CONSULTOR' && {
      ...
      icon: 'contacts',
      label: 'Clientes',
      active: location.pathname.startsWith('/consultant/customers'),
    },
    user.role === 'CONSULTOR' && {
      to: '/consultant/calls',
      icon: 'call',
      label: 'Llamadas',
      active: location.pathname.startsWith('/consultant/calls'),
    },
    user.role === 'CONSULTOR' && {
      to: '/consultant/sales',
      icon: 'point_of_sale',
      label: 'Ventas',
      active: location.pathname.startsWith('/consultant/sales'),
    },
    user.role === 'DIRECTOR' && {
    ...
    <nav className="fixed left-0 top-0 flex flex-col py-6 px-4 bg-surface-container border-r border-outline-variant/20 h-screen w-72 z-50 transition-all duration-300 ease-in-out hidden md:flex">
      ...
      <div className="space-y-1 flex-1 animate-slide-up delay-200">
        ...
        {navItems.map((item) => (
          <Link
            key={item.to}
            to={item.to}
            className={`flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 ease-in-out font-inter text-sm font-bold ${
              item.active
      ...
      <div className="mt-auto px-4 animate-scale-in delay-300">
        <div className="p-5 bg-primary-container/40 rounded-2xl border border-primary/10 backdrop-blur-sm">
          ...
          <div className="w-full py-2.5 bg-surface-container-lowest text-primary text-[10px] font-bold uppercase tracking-wider rounded-xl shadow-sm text-center">
            {user.role}
          </div>
        </div>
      </div>
